...
name: book-to-skill
description: >
  Convert a book or document (PDF, EPUB, DOCX, HTML, Markdown, plain text, RTF) into a
  structured agent skill — frameworks, mental models, principles, techniques, and
  anti-patterns — ready to study, reference, and use while working. Use when the user
  wants to turn a document into a reusable knowledge base for agents, apply an author's
  frameworks while working, or study a book. Depends on the text-extraction skill for
  step 2.
version: 1.0.0
license: MIT
metadata:
...
Books contain crystallized expertise: frameworks, principles, and techniques that took years to develop. This skill extracts that knowledge into a format agents can leverage repeatedly.
...
**Trigger:** User provides one or more new source paths and indicates they want to update an existing skill (by pointing to the existing skill folder, providing a skill slug that already exists in `SKILLS_HOME`, or explicitly requesting an update).
...
This skill runs from the mortlake skills tree. Generated book skills live in `$SKILLS_HOME`.

```bash
SKILLS_HOME="${SKILLS_HOME:-/home/john/src/mortlake/skills/knowledge}"
# extractor helper comes from the text-extraction skill:
EXTRACT_SH="/home/john/src/mortlake/skills/knowledge/text-extraction/scripts/extract.sh"
```

The mortlake skills root is registered as `external_dirs` in the Hermes config and read
by Pi, so any skill written here is immediately discoverable by all agents. Generated
book skills go under `$SKILLS_HOME/` — ask the user whether they want them in the
`knowledge/` category or elsewhere in the tree, but default to `knowledge/`.
...
> "book-to-skill requires a supported document path, folder, or glob pattern. Usage: `book-to-skill <path> [skill-name-slug]`"
...
For directories and globs, expand them to find matching supported files (`.pdf`, `.epub`, `.docx`, `.txt`, `.md`, `.markdown`, `.rst`, `.adoc`, `.html`, `.htm`, `.rtf`).
...
> "📐 Technical mode selected — using docling for structure-aware extraction (tables, code blocks, formulas preserved as markdown) if available. This takes ~1.5s per page, so expect a few minutes for longer sources. Starting now…"

**If `BOOK_TYPE=text`**, inform:
> "📄 Text mode selected — using the fastest suitable extractor for each file type. Plain text/Markdown are ready in seconds; PDFs use pandoc/pdftotext."

---

## Step 2 — Extract text from the source documents

Load the **text-extraction** skill and run its extractor, passing the input paths:

```bash
"$EXTRACT_SH" $INPUT_PATHS --mode <BOOK_TYPE>
```

Before extraction, the extractor checks for optional tools needed for each detected
format. It falls back gracefully; `--check` prints a per-format availability report.

This creates:
- `/tmp/extract_work/full_text.txt` — combined extracted text of all sources with clear visually demarcated boundaries.
- `/tmp/extract_work/metadata.json` — overall combined size, words, pages, token counts, and a detailed list of individual processed `sources`.

Read `metadata.json` to inspect the results.

---

## Step 2.5 — Pre-flight cost estimate

Read `/tmp/extract_work/metadata.json` and present the user with an estimate **before doing any generation**:
...
- Per-chapter budget midpoint by `BOOK_TYPE` (DEPTH is decided later in Step 4 and can raise it): `text` ≈ 1,000, `technical` ≈ 1,800.
...
Treat `full_text.txt` as a queryable corpus, not a single read. Loading the whole file into context burns budget you will need later for generation.
...
Use this approach for Step 3 (structure analysis), Step 7 (per-chapter summaries), and Step 8 (glossary / patterns extraction). On books under 50k tokens, a single full read is fine.
...
<!-- Preserve the most important tables as markdown. -->

## Worked Example *(DEPTH=study only — omit for DEPTH=reference)*
<!-- Reproduce one worked example/artifact from the chapter. -->
...
- **With a topic** — ask about `<topic>`; I find and read the relevant chapter
...
Commit the new skill to the mortlake repo (or leave it for the user to review first —
ask if unsure).
```
...
Analyze the new extracted text in `/tmp/extract_work/full_text.txt` to identify if the new content represents:
...
Proceed to **Step 10** to perform cleanup and print a custom update report summarizing the newly added chapters, merged glossary terms, and updated indices.

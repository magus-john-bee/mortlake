...
description: >
  ...
  Fetch the docs before answering, rather than relying on training knowledge.
---

# Get API Docs

When you need documentation for a library or API, fetch it rather than guessing
from training data. Training knowledge is stale; live docs are current.

## Retrieval strategy (try in order)

1. **llms.txt** -- Check `https://<library-site>/llms.txt` first. Many major
   libraries (Anthropic, Stripe, Vercel, Cloudflare) publish one. It gives a
   curated index of their best docs, often more reliable than web search.
2. **ProContext MCP** (when set up) -- `resolve` library name, then `search`/`read`
   for version-specific docs. Self-hosted, 2,000+ libraries. Fallback for anything
   not in llms.txt.
3. **chub CLI** -- Search and fetch from the community registry (see below).
4. **Exa MCP** -- `get_code_context_exa` for API usage patterns and examples,
   `web_search_exa` + `web_fetch_exa` for general doc pages.
5. **Web search** -- Fallback for anything not in the above.

If one source doesn't have what you need, move to the next. Don't stop at a
dead end -- try another path.
...
- IDs are `owner/repo` -- confirm the ID from search before fetching
- If multiple languages exist and you don't pass `--lang`, chub will tell you which are available
- For the full landscape of documentation retrieval tools (Context7, ProContext, DeepWiki,
  GitMCP, llms.txt standard, codebase indexing tools), see `references/documentation-retrieval-landscape.md`

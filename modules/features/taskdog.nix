...
}:
{
  flake.nixosModules.taskdog =
    ...
    let
      ...
      clientKeySecret = clientKeyForHost.${config.networking.hostName} or (
        throw "taskdog: no client API key mapped for host '${config.networking.hostName}'"
      );
